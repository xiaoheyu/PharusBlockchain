...
export class Service1Model2FormComponent implements OnInit,AfterViewChecked {
  ...
  successAjax(data){
    ...
    for (let i of data.account_list){
      if (Number(i['balance'])>=this.modelTwoPrice){
        modelOneList.push(i);
      }
    };
    $("option").remove();
    ...
    $('#purchase').submit(function(event){
      ...
      var posting =$.get(url,{address_a:$('#accountSelect').val(), trans_value:this.modelTwoPrice});
      posting.done(function(data){
        ...
        $('ol').append('<li><a class="list-group-item">\n<div class=\"bmd-list-group-col\">'+'<p class=\"list-group-item-heading\"> Transaction Address: '+data+'</p>'+'<p class=\"list-group-item-text\">'+this.modelTwoPrice+'</p></div></a></li>');
      });
    });
